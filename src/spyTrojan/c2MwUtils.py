...
class CmdRunner(threading.Thread):
    ...
    def runCmdParallel(self, cmdStr):
        """ Add a command in the paralleled execution queue."""
        if not cmdStr:
...
class programRcd(object):
    ...
    def __init__(self, uniqid, ipaddr, taskList=None, srvFlag=False) -> None:
        """ Init example :
            ...
            Args:
                ...
                tasksList (list(dict()), optional): malware preset task list. Defaults to None.
                srvFlag (bool): flag to identify whether it is a server record.
                - One task dict() example:
                {
                    'taskID': 1,
                    'taskType': 'upload',
                    'startT': None,
    ...
    def addNewTask(self, taskDict):
        """ Add a new task in to the task list.
            Args: taskDict (dict): refer to the task dict() example.
        """
...
class mwServerRcd(programRcd):
    ...
    def _initRegister(self):
        """ Init check whether the malware's 1st task is register to the c2 Hub."""
        for i, task in enumerate(self.taskList):
...
class c2TestMalware(object):
    ...
    def _initActionHandlers(self):
        """ Init the action handlers, children class can overwrite this function 
    ...
    def _startSubThreads(self):
        """ Call the start() of the handlers if they need to start sub-thread, children 
            class can overwrite this function start the user defined service. 
            > This function is called in the super().__init__(...)  
        """
        return None

    def _stopSubThreads(self):
        """ Call the stop() of the handlers if they need to stop sub-thread, children 
            class can overwrite this function stop the user defined service.
    ...
    def run(self):
        ...
        while not self.terminate:
            # Check whether got new incoming task
            task = self.c2Connector.getOneC2Task()
            # synchronized the task record
            if task is not None:
